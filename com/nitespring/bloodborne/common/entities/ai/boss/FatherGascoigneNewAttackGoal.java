...
public class FatherGascoigneNewAttackGoal extends Goal {
	   ...
	   public FatherGascoigneNewAttackGoal(FatherGascoigneBossEntity p_i1636_1_, double p_i1636_2_, boolean p_i1636_4_) {
	      this.mob = p_i1636_1_;
	   ...
	   public void tick() {
		   ...
		   LivingEntity target = this.mob.getTarget();
		      ...
		      Vec2 aim2d = MathHelpers.OrizontalAimVector(MathHelpers.AimVector(this.mob.position(), target.position()));
		      //double pi = Math.PI;
		 ...
		 //attack ticks
		   ...
		   protected void tickShootGun(Vec2 aim2d){
			 ...
			 if(animTime<=1) {
				 this.doMovement(this.mob.getTarget(), this.mob.distanceToSqr(this.mob.getTarget().getX(), this.mob.getTarget().getY(), this.mob.getTarget().getZ()));
				 this.mob.getMoveControl().setWantedPosition(this.mob.getTarget().getX(), this.mob.getTarget().getY(), this.mob.getTarget().getZ(), 0.1);
			 }
			 if(animTime==2) {
				 this.mob.getNavigation().stop();
			 ...
			 if(animTime>=15) {
			   ...
			   this.mob.setAnimationState(0);
			   this.resetAttackCooldown();
			   this.mob.getMoveControl().setWantedPosition(this.mob.getTarget().getX(), this.mob.getTarget().getY(), this.mob.getTarget().getZ(), 0.6);
			 }  
		   }
		  ...
		  protected void performLightAttack() {
		    	    ...
		    	    AttackHitboxHelpers.LargeAttack(DamageSource.mobAttack(mob),8.0f, 0.4f, mob, pos, 3.5f, -Math.PI/4, Math.PI/4, 1.0f, 3.0f);
		  ...
		  protected void performLargeSweepAttack() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttack(DamageSource.mobAttack(mob),12.0f,1.6f, mob, pos, 4.5f, -Math.PI/2, Math.PI/2, 1.0f, 3.0f);
		  ...
		  protected void performSweepAttackBehind() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttack(DamageSource.mobAttack(mob),10.0f,2.0f, mob, pos, 3.5f, Math.PI/4, -Math.PI/4, 1.0f, 3.0f);
		  ...
		  protected void performRangeAttack(Vec2 aim) {
			   ...
			   Vec3 pos = playerIn.position();
	  		  ...
	  		  Random rng = playerIn.getRandom();
	  		  int sizeVar = rng.nextInt(5) -2;
	  		  
	  		  int nBullets = 10 + sizeVar;
	 		    for(int i = -nBullets/2; i < nBullets/2; i++) {
	 		    	Random rng1 = playerIn.getRandom();
	 		    	Random rng2 = playerIn.getRandom();
