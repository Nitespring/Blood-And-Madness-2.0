...
public class GascoigneBeastAttackGoal extends Goal {
	   ...
	   public void tick() {
		   ...
		   protected void tickInAir(){
			 ...
			 if(this.mob.isOnGround()||animTime>=500) {
			   animTime=0;
			       performLightAttack();
				   this.mob.setAnimationState(0); 
		 ...
		 //attack ticks
		   ...
		   protected void tickJumpAttack(Vec2 aim2d){
			 ...
			 if(animTime>=8&&animTime<=12) {
				this.mob.setDeltaMovement(0, 1.5, 0);
			 }  
			 if(animTime>14&&animTime<=17) {
		  ...
		  protected void performLightAttack() {
		    	    ...
		    	    AttackHitboxHelpers.LargeAttack(DamageSource.mobAttack(mob),8.0f, 0.6f, mob, pos, 5.0f, -Math.PI/4, Math.PI/4, 1.0f, 3.0f);
		  ...
		  protected void performLargeSweepAttack() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttack(DamageSource.mobAttack(mob),10.0f,1.6f, mob, pos, 3.0f, -3/2*Math.PI, 3/2*Math.PI, 1.0f, 3.0f);
