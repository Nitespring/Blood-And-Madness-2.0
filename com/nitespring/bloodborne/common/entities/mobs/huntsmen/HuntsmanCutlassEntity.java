...
public class HuntsmanCutlassEntity extends AbstractHuntsmanEntity implements IAnimatable, IAnimationTickable{
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		...
		case 21:
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.atk_r1", false));
			break;
		case 22:
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.atk_r2", false));
			break;
		case 23:
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.atk_h1", false));
			break;
		case 24:
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.atk_h2", false));
			break;
		default:
			//switch(combatState) {
			//case 0:
			if(this.isAggressive()) {
				if(!(event.getLimbSwingAmount() > -0.03F && event.getLimbSwingAmount() < 0.03F)){
				 	event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.ag_walk", true));
			 	}else {
				 	event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.ag_idle", true)); 
			 	}
			 	//break;
			}else {
			 //default:
				if(!(event.getLimbSwingAmount() > -0.03F && event.getLimbSwingAmount() < 0.03F)){
					event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.walk", true));
				 }else {
					event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.huntsman_cutlass.idle", true)); 
				 } 
				 //break;
			//}
			}
	...
	public void registerControllers(AnimationData data) {
		data.addAnimationController(new AnimationController<>(this, "main_controller", 1, this::predicate));
	...
	private class MeleeAttackGoal extends Goal{
		 ...
		 protected final HuntsmanCutlassEntity mob;
		   ...
		   protected void performLightAttack() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttackWithTargetCheck(DamageSource.mobAttack(mob),5.0f, 0.1f, mob, pos, 2.5f, -Math.PI/5, Math.PI/5, -1.0f, 3.0f);
		   ...
		   protected void performHeavyAttack() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttackWithTargetCheck(DamageSource.mobAttack(mob),7.0f, 0.3f, mob, pos, 3.0f, -Math.PI/4, Math.PI/4, -1.0f, 3.5f);
