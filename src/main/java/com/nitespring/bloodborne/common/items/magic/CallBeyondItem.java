...
public class CallBeyondItem extends Item{
	...
	public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
		...
		((Player)playerIn).getCooldowns().addCooldown(playerIn.getItemInHand(handIn).getItem(), 75);
	...
	public void shoot(Player playerIn, Level worldIn){
		...
		for(int i=0; i<=5; i++) {
			...
			for(int k=0; k<=5; k++) {
				...
				CallBeyondProjectileEntity bullet = new CallBeyondProjectileEntity(ProjectileInit.CALL_BEYOND.get(), worldIn, 14.0f);
				bullet.setPos(x, y, z);
