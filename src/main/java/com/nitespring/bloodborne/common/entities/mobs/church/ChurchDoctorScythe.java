...
public class ChurchDoctorScythe extends AbstractBloodborneEntity implements GeoEntity{

	protected AnimatableInstanceCache factory = GeckoLibUtil.createInstanceCache(this);
	
	public ChurchDoctorScythe(EntityType<? extends PathfinderMob> p_i48575_1_, Level p_i48575_2_) {
	...
	private <E extends GeoAnimatable> PlayState predicate(AnimationState<E> event) {
		...
		if(this.isDeadOrDying()) {
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_scythe.death"));
		}else {
		switch(animState) {
		
		case 21:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_scythe.attack"));
			break;
		default:
			 if(!(event.getLimbSwingAmount() > -0.06F && event.getLimbSwingAmount() < 0.06F)){
				 if(this.isAggressive()) {
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_scythe.walk1"));
				 }else {
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_scythe.walk"));	 
				 }
				 }else {
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_scythe.idle"));
			 }
			break;
	...
	public int bloodRes() {return 200;}
	 ...
	 @Override
		protected void registerGoals() {
			...
			this.goalSelector.addGoal(1, new ChurchDoctorScythe.AttackGoal(this));
	 ...
	 protected SoundEvent getAmbientSound() {
	   ...
	   public static boolean checkChurchDoctorScytheSpawnRules(EntityType<? extends AbstractBloodborneEntity> e, 
	     ...
	     {  	 
	    	return checkBnMMonsterSpawnRules(e, access, type, pos, r) && 
	    			CommonConfig.should_church_doctor_scythe_spawn.get();
	     }
	   ...
	   @Override
		public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_21434_, DifficultyInstance p_21435_,
			  ...
			  this.getAttributes().getInstance(Attributes.MAX_HEALTH).setBaseValue(CommonConfig.church_doctor_scythe_hp.get());;
	 ...
	 public class AttackGoal extends Goal{
		   ...
		   protected final ChurchDoctorScythe mob;
		   private Path path;
		  ...
		  public AttackGoal(ChurchDoctorScythe entityIn) {
		      this.mob = entityIn;
	    ...
	    protected void performAttack() {
			...
			Vec3 pos = mob.position();
	   	    ...
	   	    AttackHitboxHelpers.LargeAttack(mob.damageSources().mobAttack(mob), 6.0f, 0.6f, mob, pos, 4.0f, -Math.PI/4, Math.PI/4, -1.0f, 3.0f, true);
