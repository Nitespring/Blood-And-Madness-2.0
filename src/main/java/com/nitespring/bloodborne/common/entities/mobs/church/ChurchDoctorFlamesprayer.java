...
public class ChurchDoctorFlamesprayer extends AbstractBloodborneEntity implements GeoEntity{
	...
	public ChurchDoctorFlamesprayer(EntityType<? extends PathfinderMob> p_i48575_1_, Level p_i48575_2_) {
	...
	private <E extends GeoAnimatable> PlayState predicate(AnimationState<E> event) {
		...
		if(this.isDeadOrDying()) {
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_flamesprayer.death"));
		}else {
		switch(animState) {
		
		case 21:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_flamesprayer.attack"));
			break;
		case 22:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_flamesprayer.attack1"));
			break;
		case 23:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_flamesprayer.shoot"));
			break;
		default:
			 if(!(event.getLimbSwingAmount() > -0.06F && event.getLimbSwingAmount() < 0.06F)){
				 
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_flamesprayer.walk"));	 
				 
				 }else {
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_flamesprayer.idle"));
			 }
			break;
	...
	public int bloodRes() {return 200;}
	 ...
	 @Override
		protected void registerGoals() {
			...
			this.goalSelector.addGoal(1, new ChurchDoctorFlamesprayer.AttackGoal(this));
	 ...
	 protected SoundEvent getAmbientSound() {
	   ...
	   public static boolean checkChurchDoctorFlamesprayerSpawnRules(EntityType<? extends AbstractBloodborneEntity> e, 
	     ...
	     {  	 
	    	return checkBnMMonsterSpawnRules(e, access, type, pos, r) && 
	    			CommonConfig.should_church_doctor_flamesprayer_spawn.get();
	     }
	   ...
	   @Override
		public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_21434_, DifficultyInstance p_21435_,
			  ...
			  this.getAttributes().getInstance(Attributes.MAX_HEALTH).setBaseValue(CommonConfig.church_doctor_flamesprayer_hp.get());;
	 ...
	 public class AttackGoal extends Goal{
		  ...
		  public AttackGoal(ChurchDoctorFlamesprayer entityIn) {
		      this.mob = entityIn;
		...
		public void tick() {
			...
			case 23:
				this.animTime++;
			      this.mob.getNavigation().setSpeedModifier(0.3f);
			      
			      //this.mob.getLookControl().setLookAt(this.mob.getTarget(), 30.0F, 30.0F);
			      if(animTime>=10&&animTime<=87) {
		...
		protected void doMovement(LivingEntity livingentity, Double d0) {
			...
			this.ticksUntilNextPathRecalculation = Math.max(this.ticksUntilNextPathRecalculation - 1, 0);
			   if ((this.followingTargetEvenIfNotSeen || this.mob.getSensing().hasLineOfSight(livingentity)) && this.ticksUntilNextPathRecalculation <= 0 && (this.pathedTargetX == 0.0D && this.pathedTargetY == 0.0D && this.pathedTargetZ == 0.0D || livingentity.distanceToSqr(this.pathedTargetX, this.pathedTargetY, this.pathedTargetZ) >= 1.0D || this.mob.getRandom().nextFloat() < 0.05F)) {
			         ...
			         this.ticksUntilNextPathRecalculation = 1 + this.mob.getRandom().nextInt(3);
			         if (this.canPenalize) {
	    ...
	    protected void performAttack() {
			...
			Vec3 pos = mob.position();
	   	    ...
	   	    AttackHitboxHelpers.LargeAttack(mob.damageSources().mobAttack(mob), 6.0f, 0.6f, mob, pos, 4.0f, -Math.PI/4, Math.PI/4, -1.0f, 3.0f);
	    ...
	    protected void performRangedAttack() {
	    	 ...
	    	 Vec3 aim1 = MathHelpers.AimVector(this.mob.position(), this.mob.getTarget().position());
				Vec3 aim = this.mob.getLookAngle().add(aim1.subtract(this.mob.getLookAngle()).scale(0.5f));
				double a = Math.PI/8;
			
				
				this.mob.playSound(SoundEvents.FIRECHARGE_USE, 0.25f, 1.0f);
				
				for (int i=-3; i<=3; i++) {
					for (int k=-1; k<=1; k++) {
						
						FlameProjectileEntity flame = new FlameProjectileEntity(ProjectileInit.FLAME.get(), this.mob.level(), 2.0f, 14);

						flame.setOwner(this.mob);
						double x = pos.x + aim.x*0.5 + 0.75*(aim.x*Math.cos(i*a)-aim.z*Math.sin(i*a));
						double z = pos.z + aim.z*0.5 + 0.75*(aim.z*Math.cos(i*a)+aim.x*Math.sin(i*a));
						double y = pos.y + 1.1 + 0.075*(aim.y+ 0.5*Math.sin(k*a));
						flame.setPos(x, y, z);
						//flame.setPos(pos);
						flame.xPower=aim.x*0.04 + 0.005*(aim.x*Math.cos(i*a)-aim.z*Math.sin(i*a));
						flame.yPower=aim.y*0.04 - 0.005 + 0.015*(Math.sin(k*a));
