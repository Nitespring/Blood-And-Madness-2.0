...
public class HuntsmanAxeEntity extends AbstractHuntsmanEntity implements GeoEntity{
	...
	private <E extends GeoAnimatable> PlayState predicate(AnimationState<E> event) {
		...
		case 21:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.huntsman_axe.atk_heavy"));
			break;
		case 22:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.huntsman_axe.atk_l1"));
			break;
		case 23:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.huntsman_axe.atk_l2"));
			break;
		case 24:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.huntsman_axe.atk_r1"));
			break;
		case 25:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.huntsman_axe.atk_r2"));
			break;
		default:
			 if(!(event.getLimbSwingAmount() > -0.06F && event.getLimbSwingAmount() < 0.06F)){
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.huntsman_axe.walk"));
			 }else {
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.huntsman_axe.idle")); 
			 }
			break;
	...
	protected void registerGoals() {
	  ...
	  public static boolean checkHuntsmanAxeSpawnRules(EntityType<? extends AbstractBloodborneEntity> e, ServerLevelAccessor access, MobSpawnType type, BlockPos pos, RandomSource r) 
	     {  	 
	    	return checkBnMMonsterSpawnRules(e, access, type, pos, r) && 
	    			CommonConfig.should_huntsman_axe_spawn.get();
	     }
	  @Override
		public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_21434_, DifficultyInstance p_21435_,
			  ...
			  this.getAttributes().getInstance(Attributes.MAX_HEALTH).setBaseValue(CommonConfig.huntsman_axe_hp.get());;
	...
	private class MeleeAttackGoal extends Goal{
		 ...
		 protected final HuntsmanAxeEntity mob;
		   ...
		   protected void performLightAttack() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttack(mob.damageSources().mobAttack(mob),6.0f, 0.2f, mob, pos, 2.5f, -Math.PI/5, Math.PI/5, -1.0f, 3.0f, true);
		   ...
		   protected void performHeavyAttack() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttack(mob.damageSources().mobAttack(mob),8.0f, 0.5f, mob, pos, 3.0f, -Math.PI/4, Math.PI/4, -1.0f, 3.5f, true);
		   ...
		   protected void performTorchAttack() {
	    	    ...
	    	    AttackHitboxHelpers.LargeAttack(mob.damageSources().source(CustomDamageSource.FIRE_DAMAGE, mob),3.0f, 0.2f, mob, pos, 2.0f, -Math.PI/5, Math.PI/5, -1.0f, 3.0f, true);
