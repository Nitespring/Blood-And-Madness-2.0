...
public class ChurchDoctorPistol extends AbstractBloodborneEntity implements GeoEntity{

	protected AnimatableInstanceCache factory = GeckoLibUtil.createInstanceCache(this);
	
	public ChurchDoctorPistol(EntityType<? extends PathfinderMob> p_i48575_1_, Level p_i48575_2_) {
	...
	private <E extends GeoAnimatable> PlayState predicate(AnimationState<E> event) {
		...
		if(this.isDeadOrDying()) {
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_pistol.death"));
		}else {
		switch(animState) {
		
		case 21:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_pistol.attack"));
			break;
		case 22:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_pistol.attack1"));
			break;
		case 23:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_pistol.shoot"));
			break;
		default:
			 if(!(event.getLimbSwingAmount() > -0.06F && event.getLimbSwingAmount() < 0.06F)){
				 
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_pistol.walk"));	 
				 
				 }else {
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_pistol.idle"));
			 }
			break;
	...
	public int bloodRes() {return 200;}
	 ...
	 @Override
		protected void registerGoals() {
			...
			this.goalSelector.addGoal(1, new ChurchDoctorPistol.AttackGoal(this));
	 ...
	 protected SoundEvent getAmbientSound() {
	   ...
	   public static boolean checkChurchDoctorPistolSpawnRules(EntityType<? extends AbstractBloodborneEntity> e, 
	     ...
	     {  	 
	    	return checkBnMMonsterSpawnRules(e, access, type, pos, r) && 
	    			CommonConfig.should_church_doctor_pistol_spawn.get();
	     } 
	   ...
	   @Override
		public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_21434_, DifficultyInstance p_21435_,
			  ...
			  this.getAttributes().getInstance(Attributes.MAX_HEALTH).setBaseValue(CommonConfig.church_doctor_pistol_hp.get());;
	 ...
	 public class AttackGoal extends Goal{
		   ...
		   protected final ChurchDoctorPistol mob;
		   private Path path;
		  ...
		  public AttackGoal(ChurchDoctorPistol entityIn) {
		      this.mob = entityIn;
		...
		public void tick() {
			...
			case 23:
				this.animTime++;
		          ...
		          if(animTime>=16) {
			    	  this.mob.setAnimationState(0);
			    	  int i = (int)(new Random().nextFloat()*50-25);
			    	  this.ticksUntilNextRangedAttack=40+i;
			    	  this.animTime = 0;
			      } 
				break;
	    ...
	    protected void performAttack() {
			...
			Vec3 pos = mob.position();
	   	    ...
	   	    AttackHitboxHelpers.LargeAttack(mob.damageSources().mobAttack(mob), 6.0f, 0.4f, mob, pos, 3.0f, -Math.PI/(4+0.5), Math.PI/(4+0.5), -1.0f, 2.0f);
		 ...
		 protected void performRangedAttack() {
			 ChurchDoctorPistol playerIn = this.mob;
			   Level worldIn = this.mob.level();
			   
			   Vec3 aim = MathHelpers.AimVector(this.mob.position(), playerIn.getTarget().position());
			   Vec3 pos = playerIn.position();
	 		    ...
	 		    for(int i = 0; i <=1; i++) {
	 		    	...
	 		    	BulletProjectileEntity bullet = new BulletProjectileEntity(ProjectileInit.BULLET_ENTITY.get(), 9.0f, 120, worldIn, this.mob);
	 		   ...
	 		   worldIn.playSound((Player)null, pos.x, pos.y, pos.z, SoundEvents.GENERIC_EXPLODE, SoundSource.HOSTILE, 0.5F, 0.4F);
	 		    	bullet.setPos(playerIn.getX() + aim.x * 0.5D + 0.2*i, playerIn.getY() + 0.75, playerIn.getZ() + aim.y * 0.5D + 0.2*i);
	 		   
	 		    	 Vec3 aim1 = MathHelpers.AimVector(bullet.position(), playerIn.getTarget().position().add( i*0.2, playerIn.getTarget().getBbHeight()/2, i*0.2));
	 				    bullet.xPower = aim1.x * 0.2;
	 				    bullet.yPower = aim1.y * 0.2;
	 				    bullet.zPower = aim1.z * 0.2;
