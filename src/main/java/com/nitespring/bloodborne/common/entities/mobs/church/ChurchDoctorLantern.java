...
public class ChurchDoctorLantern extends AbstractBloodborneEntity implements GeoEntity{
	...
	private <E extends GeoAnimatable> PlayState predicate(AnimationState<E> event) {
		...
		if(this.isDeadOrDying()) {
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_lantern.death"));
		}else {
		switch(animState) {
		
		case 21:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_lantern.attack"));
			break;
		case 22:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_lantern.attack1"));
			break;
		case 23:
			event.getController().setAnimation(RawAnimation.begin().thenPlay("animation.church_doctor_lantern.attack2"));
			break;
		default:
			 if(!(event.getLimbSwingAmount() > -0.06F && event.getLimbSwingAmount() < 0.06F)){
				 
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_lantern.walk"));	 
				 
				 }else {
				 event.getController().setAnimation(RawAnimation.begin().thenLoop("animation.church_doctor_lantern.idle"));
			 }
			break;
	...
	public int bloodRes() {return 200;}
	 ...
	 @Override
		protected void registerGoals() {
			...
			this.goalSelector.addGoal(1, new ChurchDoctorLantern.AttackGoal(this));
	 ...
	 protected SoundEvent getAmbientSound() {
	   ...
	   public static boolean checkChurchDoctorLanternSpawnRules(EntityType<? extends AbstractBloodborneEntity> e, 
	     ...
	     {  	 
	    	return checkBnMMonsterSpawnRules(e, access, type, pos, r) && 
	    			CommonConfig.should_church_doctor_lantern_spawn.get();
	     }
	   ...
	   @Override
		public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_21434_, DifficultyInstance p_21435_,
			  ...
			  this.getAttributes().getInstance(Attributes.MAX_HEALTH).setBaseValue(CommonConfig.church_doctor_lantern_hp.get());;
	 ...
	 public class AttackGoal extends Goal{
	    ...
	    protected void performAttack() {
			...
			Vec3 pos = mob.position();
	   	    ...
	   	    AttackHitboxHelpers.LargeAttack(mob.damageSources().mobAttack(mob), 5.0f, 0.4f, mob, pos, 3.0f, -Math.PI/4, Math.PI/4, -1.0f, 2.0f);
